...
<head>
    ...
    <title><?= lang("can_req_micro_list"); ?></title>
...
<?php $this->load->view('frontend/layout/navbar'); ?>
    <div class="container">
        <div class="row">     
            <div class="col-sm-12">                          
                <h3 class="text-center text-warning mb-5"><?= lang("can_req_micro_list"); ?></h3>
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body text-success">
                            <?php if ($this->session->flashdata('add_success')){ ?>
                                <div class="row">
                                    <div class="col-12"><?= lang("can_micro_add_success"); ?></div>
                                </div>
                            <?php } ?>
                            <?php if ($this->session->flashdata('cancel_success')){ ?>
                                <div class="row">
                                    <div class="col-12"><?= lang("can_micro_cancel_success"); ?></div>
                                </div>
                            <?php } ?>
                            <?php if ($this->session->flashdata('accept_success')){ ?>
                                <div class="row">
                                    <div class="col-12"><?= lang("can_micro_accept_success"); ?></div>
                                </div>
                            <?php } ?>
                            <?php if ($this->session->flashdata('complain_success')){ ?>
                                <div class="row">
                                    <div class="col-12"><?= lang("can_micro_complain_success"); ?></div>
                                </div>
                            <?php } ?>
                        </div>
                        <div class="modal-footer justify-content-center">
    ...
    <script>
        ...
        function cancel(){
            window.location = "<?= base_url(); ?>frontend/Requestmicrochip/cancel/"+cancelId;
        }
        function confirm(id){
            cancelId = id;
        ...
        function accept(id){
            window.location = "<?= base_url(); ?>frontend/Requestmicrochip/accept/"+id;
        }
        function complain(id){
            window.location = "<?= base_url(); ?>frontend/Requestmicrochip/complain/"+id;
        }
