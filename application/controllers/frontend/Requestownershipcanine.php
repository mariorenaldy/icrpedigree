				...
				if ($this->form_validation->run() == FALSE){
					...
					if (!$err){
						...
						if ($res){
							...
							$this->session->set_flashdata('error_message', 'Gagal menyimpan laporan ubah pemilik canine.');
							$this->load->view("frontend/add_request_ownership", $data);
						}
					}
					else{
						$this->db->trans_rollback();
						$this->session->set_flashdata('error_message', 'Gagal menyimpan laporan ubah pemilik canine. Error code : '.$err);
						$this->load->view("frontend/add_request_ownership", $data);
					}
				}
			}
