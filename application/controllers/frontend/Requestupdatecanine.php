				...
				if ($this->form_validation->run() == FALSE){
					...
					if (!$err){
						...
						if ($res){
							...
							$this->session->set_flashdata('error_message', 'Gagal menyimpan laporan ubah foto canine.');
							$this->load->view("frontend/add_request_update_canine", $data);
						}
					}
					else{
						$this->session->set_flashdata('error_message', 'Gagal menyimpan laporan ubah foto canine. Error code : '.$err);
						$this->load->view("frontend/add_request_update_canine", $data);
					}
				}
			}
