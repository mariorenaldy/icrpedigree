			...
			if ($this->uri->segment(4)){
				if ($this->session->userdata('use_username')){
					...
					if ($err){
						...
						$this->session->set_flashdata('error_message', 'Failed to approve change canine photo id = '.$this->uri->segment(4).'. Err code: '.$err);
						redirect('backend/Requestupdatecanine');
					}
				}
				else{
		...
		public function reject(){
			if ($this->uri->segment(4)){
				if ($this->session->userdata('use_username')){
					...
					if ($err){
						$this->db->trans_rollback();
						$this->session->set_flashdata('error_message', 'Failed to reject change canine photo id = '.$this->uri->segment(4).'. Err code: '.$err);
						redirect('backend/Requestupdatecanine');
					}
				}
				else{
