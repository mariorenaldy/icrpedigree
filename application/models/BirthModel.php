...
class BirthModel extends CI_Model {
    ...
    public function get_births($where, $offset = 0, $limit = 1){
        $this->db->select('*, DATE_FORMAT(bir_date_of_birth, "%d-%m-%Y") as bir_date_of_birth, DATE_FORMAT(bir_app_date, "%d-%m-%Y") as bir_app_date, c1.can_a_s AS sire, c2.can_a_s AS dam');
        $this->db->from('births');
        if ($where != null) {
            $this->db->where($where);
        }
