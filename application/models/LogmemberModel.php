...
class LogmemberModel extends CI_Model {
    ...
    public function get_logs($where){
        $this->db->select('*, DATE_FORMAT(logs_member.log_payment_date, "%d-%m-%Y") as log_payment_date, DATE_FORMAT(logs_member.log_date, "%d-%m-%Y %H:%i:%s") as log_date, DATE_FORMAT(logs_member.log_app_date, "%d-%m-%Y") as log_app_date, u1.use_username AS user, u2.use_username AS app_user');
