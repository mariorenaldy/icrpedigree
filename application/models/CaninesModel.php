...
class CaninesModel extends CI_Model {
    ...
    public function search_app($q){
        ...
        $sql = "select * from canines c, members m, kennels k where c.can_member_id = m.mem_id AND k.ken_member_id = m.mem_id AND k.ken_id = c.can_kennel_id AND c.can_stat = 1 AND (c.can_icr_number LIKE '%".$q."%' OR c.can_icr_moc_number LIKE '%".$q."%' OR c.can_a_s LIKE '%".$q."%' OR c.can_cage LIKE '%".$q."%'";
        if ($date)
            $sql .= " OR c.can_date_of_birth LIKE '%".$date."%')";
        else
            $sql .= ")";
