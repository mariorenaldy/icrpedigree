...
function send_birth_link($email, $member, $sire, $dam, $id){
	...
	$message .= '<div><a href="'.base_url().'frontend/Births/add/'.$id.'" style="color: #E8A317;">Lapor Lahir</a></div><br>';
	...
	$message .= '<div><a href="'.base_url().'frontend/Births/add/'.$id.'" style="color: #E8A317;">Report Birth</a></div><br>';
...
function send_stambum_link($email, $member, $sire, $dam, $id){
	...
	$message .= '<hr style="border-top: 4px solid black; margin-bottom: 1em;">';
	$message .= '<div>Yth. <b>'.$member.'</b>,</div><br>';
	$message .= '<div>Kami dari tim ICRPedigree ingin memberitahukan Anda bahwa pendaftaran kelahiran anjing hasil pacak antara <b>'.$sire.'</b> dan <b>'.$dam.'</b> telah disetujui. Langkah selanjutnya adalah mendaftarkan nama anak dan pembuatan stambum.</div><br>';
	$message .= '<div><a href="'.base_url().'frontend/Stambums/add/'.$id.'" style="color: #E8A317;">Lapor Anak</a></div><br>';
	$message .= '<div>Mohon diperhatikan bahwa pendaftaran nama anak dan pembuatan stambum harus dilakukan minimal <b>'.$ci->config->item('min_jarak_lapor_anak').' hari</b> setelah tanggal lahir dan batas maksimum <b>'.$ci->config->item('jarak_lapor_anak').' hari</b> dari lahir. Pastikan Anda mematuhi jangka waktu ini untuk memastikan bahwa pendaftaran dilakukan dengan tepat waktu.</div><br>';
	...
	$message .= '<hr style="border-top: 4px solid black; margin-top: 1em; margin-bottom: 1em;">';
	$message .= '<div>Dear <b>'.$member.'</b>,</div><br>';
	$message .= '<div>We, the ICRPedigree team, would like to inform you that the registration of birth from the stud between <b>'.$sire.'</b> and <b>'.$dam.'</b> has been approved. The next step is to register the names of the puppies and create a stambum.</div><br>';
	$message .= '<div><a href="'.base_url().'frontend/Stambums/add/'.$id.'" style="color: #E8A317;">Report Puppy</a></div><br>';
	$message .= '<div>Please note that registration of puppies names and stambum creation must be done at least <b>'.$ci->config->item('min_jarak_lapor_anak').' days</b> after the date of birth and a maximum of <b>'.$ci->config->item('jarak_lapor_anak').' days</b> from birth. Make sure you comply with this timeframe to ensure that registration is done in a timely manner.</div><br>';
	$message .= "<div>We would like to remind you to ensure that all information provided in registration is accurate and in accordance with actual data. This will assist us in ensuring that the registered dog's details are printed correctly on the stambum.</div><br>";
