...
/* /common/init.js*/
/**
 * @preserve Basic Primitives Diagram v3.7.4
...
primitives.orgdiagram.ConnectionsGraphTask = function (getGraphics, createTranfromTask, connectorsOptionTask, visualTreeLevelsTask, alignDiagramTask) {
	...
	function ConnectorEdge(from, to, polyline, parentsArrowId, childrenArrowId, dotId, weight, fromOffset) {
		this.polyline = polyline;
		this.from = from;
		this.to = to;
	...
	function traceVerticalBundle(graph, bundle, hasGraphics) {
		...
		if (parents.length > 0) {
			...
			traceTopConnectors(graph, topCenterPoint, parents, true, hasGraphics, true, bundle.fromOffset);
		}
		...
		if (children.length > 0) {
			...
			traceTopConnectors(graph, bottomCenterPoint, children, hasSquared, hasGraphics, false);
		}
	...
	function traceTopConnectors(graph, parentPoint, points, hasSquared, hasGraphics, isParents, fromOffset) {
