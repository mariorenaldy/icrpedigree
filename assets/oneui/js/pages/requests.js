...
var BaseTableDatatables = function() {
    ...
    var initDataTablemember = function() {
        window.tablemember = jQuery('.data-requests').dataTable({
            ...
            serverSide: true,
            ajax: {
              url: base_url+'backend/canines/request_data',
              type: 'POST',
            },
            columns: [
                    { data: 'req_can_photo',
                        render: function(data, type, row) {
                            if (data == '-') {
                                ...
                                if (row.can_photo == '-') {
                                    ...
                                    str += '<img src="'+base_url+'uploads/canine/'+ row.can_photo + '" width="100" class="img img-thumbnail" style="border-radius:5%">';
                                }
                    ...
                    { data: 'req_id',
                      render: function(data, type, row) {
                          if (row.use_username == ''){
                            var str = '<button class="btn btn-success" onClick="approve(\''+ data + '\')" data-toggle="tooltip" title="Approve Request"><i class="si si-check"></i></button>';
                            return str;
                          }
                          else{
                              return '';
                    ...
                    { data: 'req_id',
                      render: function(data, type, row) {
                          if (row.use_username == ''){
                            var str = '<button class="btn btn-danger" onClick="reject(\''+ data + '\')" data-toggle="tooltip" title="Reject Request"><i class="si si-close"></i></button>';
                            return str;
                          }
                          else{
                              return '';
